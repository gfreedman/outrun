...
Extracts individual cactus sprites from source/cactus.png.
...
cactus varieties arranged in rows.  This script:
  ...
  7. Stitches into a horizontal atlas: dist/cactus_sheet.png.
...
os.makedirs("parts/cactuses", exist_ok=True)
...
for name, (label_idx, area, x0, y0, x1, y1) in zip(NAMES, groups):
    ...
    out_path = f"parts/cactuses/{name.lower()}.png"
    img.save(out_path)
...
sheet.save("dist/cactus_sheet.png")
print(f"Saved dist/cactus_sheet.png  {sheet_w}×{sheet_h}")

...
Run from the repo root (the directory that contains the ``assets/`` folder):
    ...
    python sprites/asset_build_scripts/build_barney_sheet.py

── Input ──────────────────────────────────────────────────────────────────

  parts/billboards/barney_boards/billboard_barney_*.png
  (2 billboard PNGs, one per entry in the ORDER list below)

── Outputs ────────────────────────────────────────────────────────────────

  dist/barney_sheet.png   Horizontal atlas — all barney boards packed into a
                            single PNG row, separated by PAD-pixel gutters.
  (stdout)                  TypeScript constant block ready to paste into
                            sprites.ts (SpriteId union lines + BARNEY_RECTS
...
# If a future version of this script adds background-removal logic, a TOLERANCE
# constant would belong here.  TOLERANCE controls the pixel-distance threshold
# (in RGBA colour space) used to decide whether a pixel belongs to the
# background colour — higher values remove more background, lower values are
# more conservative and preserve edge anti-aliasing.  The typical starting
# value is 30 (out of 255) which handles JPEG compression artefacts without
# eroding intentional near-background colours.

# Ordered list of (SpriteId-suffix, source-file) pairs.  Atlas columns appear
# left-to-right in this exact order; TypeScript BARNEY_RECTS preserves the
# same order so index-based iteration in sprites.ts is stable.
ORDER = [
...
sheet.save("dist/barney_sheet.png")
print(f"Saved dist/barney_sheet.png  {sheet_w}x{sheet_h}")
print()

# ── Print TypeScript constants ─────────────────────────────────────────────────

# Emit a ready-to-paste TypeScript block: SpriteId union additions first,
# then the BARNEY_RECTS export, then human-readable verification lines.
